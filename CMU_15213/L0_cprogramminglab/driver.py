...
class Tracer:
    ...
    def runTrace(self, trace_id):
        if trace_id not in self.traceDict:
            print(f"ERROR: No trace with id {trace_id}")
            return False

        fname = os.path.join(self.traceDirectory, f"{self.traceDict[trace_id]}.cmd")
        clist = [
            self.qtest,
            "-v",
            f"{self.verbLevel}",
    ...
    def run(self, tid=None):
        ...
        if self.autograde:
            ...
            scores = collections.OrderedDict(
                [(self.traceProbs[k], score) for (k, score) in scoreDict.items()]
            )
            print(json.dumps(dict(scores=scores)))


def run():
    parser = argparse.ArgumentParser(description="Runs the cprogramminglab driver")
    parser.add_argument("-p", metavar="PROG", default="./qtest", help="Program to test")
    parser.add_argument("-t", metavar="TID", type=int, help="Trace ID to test")
    parser.add_argument(
        "-v",
        metavar="VLEVEL",
        type=int,
